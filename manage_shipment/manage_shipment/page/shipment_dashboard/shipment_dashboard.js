frappe.pages['shipment-dashboard'].on_page_load = function(wrapper) {
    ...
    function render_table(rows) {
        ...
        const body=rows.map(r=>`<tr><td><input type="checkbox" class="shipment-select" data-name="${frappe.utils.escape_html(r.name)}"></td><td><a href="/app/shipment/${encodeURIComponent(r.name)}">${frappe.utils.escape_html(r.tracking_id||r.name)}</a><div class="text-muted small">${frappe.utils.escape_html(r.courier_service_provider_name||'')}</div></td><td>${frappe.utils.escape_html(r.consignee_name||r.customer||'')}</td><td><span class="indicator-pill ${status_class(r.status)}">${frappe.utils.escape_html(r.status||'')}</span><div class="text-muted small">${frappe.utils.escape_html(r.courier_status||'')}</div></td><td>${frappe.utils.escape_html(r.current_location||'')}</td><td>${frappe.utils.escape_html(r.expected_delivery_date||'')}<div class="text-muted small">${frappe.utils.escape_html(r.last_tracked_on||'')}</div></td><td>${r.follow_up_required?'<span class="indicator-pill red">Follow-up</span>':''}${r.follow_up_overdue?'<span class="indicator-pill orange ml-1">Overdue</span>':''}</td></tr>`).join('');
    ...
    frappe.call({method:'frappe.client.get_list',args:{doctype:'Courier Service Provider',fields:['name','provider_name'],filters:{enabled:1},limit_page_length:100},callback(r){courier.df.options=[''].concat((r.message||[]).map(x=>x.name));courier.refresh();load();}});
